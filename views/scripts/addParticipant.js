...
function add_function(event) {
    ...
    if(intId < $(this).attr("maxSize")) {
        ...
        var pERelation = $(`<select id="part${intId}erelation" name="part${intId}erelation" class = "relation" form = "eventSignupForm" required><option selected disabled hidden style='display: none' value=''></option><option value="parent">Parent/Guardian</option><option value="spouse">Spouse</option><option value="relative">Relative</option><option value="friend">Friend</option></select><br>`);
